  ...
  return (
    <div className="min-h-screen flex relative">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 min-h-screen relative" style={{ backgroundColor: 'var(--color-background)' }}>
        {/* D.1 — theme toggle anchored to the right panel (not the
            viewport), so it sits within the page chrome the way it does
            on /about, /privacy, /contact rather than floating in the
            viewport corner. */}
          ...
          {forgotMode ? (
            ...
            <>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 pr-10 rounded-lg border border-border bg-surface text-ink text-sm
                        ...
                        focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                    />
                    <button type="button" onClick={() => setShowPassword((s) => !s)} aria-label={showPassword ? 'Hide password' : 'Show password'} aria-pressed={showPassword} className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-muted hover:text-ink-secondary" tabIndex={-1}>
              ...
              <p className="mt-6 text-center text-sm text-ink-muted">
                ...
                <Link to="/signup" className="hover:underline cursor-pointer" style={{ color: 'var(--color-primary)', fontWeight: 800 }}>
                  Create one
                </Link>
              </p>
