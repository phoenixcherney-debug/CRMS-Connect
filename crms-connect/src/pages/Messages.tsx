...
export default function Messages() {
  ...
  async function load() {
    ...
    for (const msg of messages ?? []) {
      if (!lastMsgMap[msg.conversation_id]) {
        lastMsgMap[msg.conversation_id] = msg as Message
      }
      if (!msg.is_read && msg.sender_id !== profile.id) {
  ...
  async function openOrCreateConversation(otherId: string) {
    ...
    const { data: created } = await supabase
      .from('conversations')
      .insert({ participant_one: profile.id, participant_two: otherId })
      .select('id')
      .single()
  ...
  function previewContent(msg: Message): string {
    try {
      ...
      if (parsed?.type === 'job_share') return `📋 ${parsed.title} at ${parsed.company}`
    } catch {}
    return msg.content
  }
